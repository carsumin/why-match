    ...
    <div className="flex flex-col">
      ...
      <section className="max-w-6xl mx-auto w-full px-4 pt-16 pb-10 text-center">
        ...
        <div className="mt-12 grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-3xl mx-auto">
          <Link
            href="/hackathons"
            className="group flex flex-col items-center gap-3 p-8 rounded-2xl bg-[#eef1fb] text-slate-800 font-bold text-lg hover:bg-[#dde4f5] transition-all duration-200 shadow-sm hover:shadow-md hover:-translate-y-0.5 animate-fade-in-up"
            style={{ animationDelay: '220ms' }}
          >
            <span className="text-4xl">🏆</span>
            <span>해커톤 보러가기</span>
            <span className="text-sm font-normal text-slate-500">진행 중·예정 해커톤 확인</span>
          </Link>
          <Link
            href="/camp"
            className="group flex flex-col items-center gap-3 p-8 rounded-2xl bg-[#eaf7f2] text-slate-800 font-bold text-lg hover:bg-[#d1f0e5] transition-all duration-200 shadow-sm hover:shadow-md hover:-translate-y-0.5 animate-fade-in-up"
            style={{ animationDelay: '320ms' }}
          >
            <span className="text-4xl">👥</span>
            <span>팀 찾기</span>
            <span className="text-sm font-normal text-slate-500">팀원 모집·합류 신청</span>
          </Link>
          <Link
            href="/rankings"
            className="group flex flex-col items-center gap-3 p-8 rounded-2xl bg-slate-100 text-slate-800 font-bold text-lg hover:bg-slate-200 transition-all duration-200 hover:-translate-y-0.5 animate-fade-in-up"
            style={{ animationDelay: '420ms' }}
          >
            <span className="text-4xl">📊</span>
            <span>랭킹 보기</span>
            <span className="text-sm font-normal text-slate-500">글로벌 참가자 순위</span>
          </Link>
        </div>
      </section>
      ...
      {ongoingHackathons.length > 0 && (
        <section className="w-full glass border-t border-[#dde4f5] py-12">
              ...
              {ongoingHackathons.map((hackathon, i) => (
                <AnimateIn key={hackathon.slug} delay={i * 80}>
                  <Link
                    ...
                    className="block p-5 rounded-2xl bg-[#eef1fb] border border-[#dde4f5] hover:border-[#4f72c4] hover:shadow-md transition-all duration-200"
                  >
      ...
      {/* 매칭 공식 소개 */}
      <section className="w-full bg-[#f0f3fb] border-t border-[#dde4f5] py-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <AnimateIn delay={0}>
              <div className="p-6 rounded-2xl glass shadow-sm">
                <div className="text-5xl font-extrabold text-[#4f72c4] mb-2 tabular-nums">
            ...
            <AnimateIn delay={100}>
              <div className="p-6 rounded-2xl glass shadow-sm">
                <div className="text-5xl font-extrabold text-[#10b981] mb-2 tabular-nums">
      ...
      {/* 데이터 없음 상태 */}
      {ongoingHackathons.length === 0 && (
        <section className="w-full border-t border-[#dde4f5] py-12">
          <div className="max-w-6xl mx-auto px-4 text-center text-gray-400">
            <p className="text-4xl mb-3">📭</p>
            <p className="text-sm">현재 진행 중인 해커톤이 없습니다.</p>
            <Link href="/hackathons" className="mt-3 inline-block text-sm text-[#4f72c4] hover:underline">
              전체 해커톤 보기 →
            </Link>
          </div>
        </section>
